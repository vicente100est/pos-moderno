...
if ($request->get['brandid']) {
  $where_query .= " AND brand_id = " . $request->get['brandid'];
}
if (from()) {
  $from = from();
  $to = to();
...
// DB table to use
$table = "(SELECT selling_info.invoice_id, selling_info.created_at, selling_item.id, selling_item.brand_id, selling_item.item_name, selling_item.item_quantity as total_item, selling_item.item_discount as discount, selling_item.item_tax as tax, selling_item.item_purchase_price as purchase_price, selling_item.item_total as sell_price FROM selling_item 
...
$columns = array(
    ...
    array( 
      ...
      'dt' => 'brand_name',
      'formatter' => function( $d, $row ) {
        return get_the_brand($row['brand_id'], 'brand_name');
      }
    ),
