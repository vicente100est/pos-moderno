window.angularApp.controller("BankAccountController", [
    "$scope", 
    "API_URL",
    "window",
    "jQuery",
...
function (
    ...
    dt.dataTable({
        ...
        "dom": "lfBrtip",
        "serverSide": true,
        "ajax": API_URL + "/_inc/bank_account.php",
        "order": [[ 0, "desc"]],
        "aLengthMenu": [
            [10, 25, 50, 100, 200, -1],
        ...
        "buttons": [
            ...
            },
            {
                ...
                titleAttr: "Copy",
                title: window.store.name + " > Bank Account List",
                exportOptions: {
                    columns: [ 0, 1, 2, 3 ]
                }
            },
    ...
    // Create new taxrate
    $(document).delegate("#create-account-submit", "click", function(e) {
        e.preventDefault();
    ...
    // Edit account
    $(document).delegate("#edit-account", "click", function(e) {
        e.stopPropagation();
        e.preventDefault();

        var d = dt.DataTable().row($(this).closest("tr") ).data();
        BankAccountEditModal(d);
    });

    // Delete account
    $(document).delegate("#delete-account", "click", function(e) {
        e.stopPropagation();
        e.preventDefault();
        var d = dt.DataTable().row( $(this).closest("tr") ).data();
        BankAccountDeleteModal(d);
    });

    // Open edit modal dialog account by query string
        ...
        dt.DataTable().ajax.reload(function(json) {
            $.each(json.data, function(index, obj) {
                if (obj.DT_RowId === "row_" + taxrateId) {
                    BankAccountEditModal({taxrate_id: taxrateId, taxrate_name: obj.taxrate_name});
                    return false;
                }
            });
        }, false);
