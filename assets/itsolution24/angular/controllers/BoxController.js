window.angularApp.controller("BoxController", [
    "$scope", 
    "API_URL",
    "window",
    "jQuery",
...
function (
    ...
    dt.dataTable({
        ...
        "columnDefs": [
            ...
            { 
                ...
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#box-box-list thead tr th:eq(0)").html());
                }
            },
            { 
                "targets": [1],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#box-box-list thead tr th:eq(1)").html());
                }
            },
            { 
                "targets": [2],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#box-box-list thead tr th:eq(2)").html());
                }
            },
            { 
                "targets": [3],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#box-box-list thead tr th:eq(3)").html());
                }
            },
            { 
                "targets": [4],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#box-box-list thead tr th:eq(4)").html());
                }
            },
            { 
                "targets": [5],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#box-box-list thead tr th:eq(5)").html());
                }
            },
            { 
                "targets": [6],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#box-box-list thead tr th:eq(6)").html());
                }
            }
        ],
        "aoColumns": [
        ...
        "buttons": [
            ...
            },
            {
                ...
                titleAttr: "Copy",
                title: window.store.name + " > Box List",
                exportOptions: {
                    columns: [ 0, 1, 2 ]
                }
            },
    ...
    // Edit box
    $(document).delegate("#edit-box", "click", function(e) {
        e.stopPropagation();
        e.preventDefault();
    ...
    // Delete box
    $(document).delegate("#delete-box", "click", function(e) {
        e.stopPropagation();
        e.preventDefault();
        var d = dt.DataTable().row( $(this).closest("tr") ).data();
        BoxDeleteModal(d);
    ...
    if (window.getParameterByName("box_id") && window.getParameterByName("box_name")) {
        boxId = window.getParameterByName("box_id");
        var boxName = window.getParameterByName("box_name");
        dt.DataTable().search(boxName).draw();
        dt.DataTable().ajax.reload(function(json) {
            $.each(json.data, function(index, obj) {
                if (obj.DT_RowId === "row_" + boxId) {
                    BoxEditModal({box_id: boxId, box_name: obj.box_name});
                    return false;
                }
            });
        }, false);
