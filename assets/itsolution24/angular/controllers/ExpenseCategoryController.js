window.angularApp.controller("ExpenseCategoryController", [
    "$scope",
    "API_URL",
    "window",
    "jQuery",
...
function (
    ...
    dt.dataTable({
        ...
        "dom": "lfBrtip",
        "serverSide": true,
        "ajax": API_URL + "/_inc/expense_category.php?from=" + $from + "&to=" + $to,
        "order": [[ 0, "desc"]],
        "aLengthMenu": [
            [10, 25, 50, 100, 200, -1],
        ...
        "buttons": [
            {
                ...
                titleAttr: "Print",
                title: window.store.name + " > Expense Category",
                exportOptions: {
                    columns: [ 0, 1, 2, 3, 4, 5 ]
                }
            },
    ...
    $(document).delegate("#create-expense-category-submit", "click", function(e) {
        ...
        then(function(response) {
            ...
            dt.DataTable().ajax.reload(function(json) {
                if ($("#row_"+expense_categoryId).length) {
                    $("#row_"+expense_categoryId).flash("yellow", 5000);
                }
            }, false);

            // Reset form
            $("#reset").trigger("click");
            $("#expense_category_sex").val(null).trigger("change");
    ...
    $(document).delegate("#edit-expense_category", "click", function(e) {
        e.stopPropagation();
        e.preventDefault();
        var d = dt.DataTable().row( $(this).closest("tr") ).data();
        ExpenseCategoryEditModal(d);
    });

    // Delete expense_category
    $(document).delegate("#delete-expense_category", "click", function(e) {
        e.stopPropagation();
        e.preventDefault();
        var datatable = dt;
        var d = datatable.DataTable().row( $(this).closest("tr") ).data();
        ExpenseCategoryDeleteModal(d);
    });
    ...
    if (window.getParameterByName("category_id") && window.getParameterByName("category_name")) {
        expense_categoryId = window.getParameterByName("category_id");
        var expense_categoryName = window.getParameterByName("category_name");
        dt.DataTable().search(expense_categoryName).draw();
        dt.DataTable().ajax.reload(function(json) {
            $.each(json.data, function(index, obj) {
                if (obj.DT_RowId === "row_" + expense_categoryId) {
                    ExpenseCategoryEditModal({category_id: expense_categoryId, category_name: obj.category_name});
                    return false;
                }
            });
        }, false);
