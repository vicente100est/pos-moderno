window.angularApp.controller("InstallmentPaymentController", [
    "$scope",
    "API_URL",
    "window",
    "jQuery",
...
function (
    ...
    $("#payment-payment-list").dataTable({
        ...
        "dom": "lfBrtip",
        "serverSide": true,
        "ajax": API_URL + "/_inc/installment_payment.php?from="+$from+"&to="+$to+"&type="+$type,
        "order": [[ 0, "desc"]],
        "aLengthMenu": [
            [10, 25, 50, 100, 200, -1],
        ...
        "columnDefs": [
            ...
            { 
                "targets": [6],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#payment-payment-list thead tr th:eq(6)").html());
                }
            },
        ],
        "aoColumns": [
        ...
        "buttons": [
            {
                ...
                titleAttr: "Print",
                title: "Installment Payment-"+from+" to "+to,
    ...
    $(document).delegate(".payment-btn", "click", function (e) {
        e.stopPropagation();
        e.preventDefault();
        var d = dt.DataTable().row( $(this).closest("tr") ).data();
        InstallmentPaymentModal(d);
    });


    // Append email button into datatable buttons
	if (window.sendReportEmail) { $(".dt-buttons").append("<button id=\"email-btn\" class=\"btn btn-default buttons-email\" tabindex=\"0\" aria-controls=\"payment-payment-list\" type=\"button\" title=\"Email\"><span><i class=\"fa fa-envelope\"></i></span></button>"); };
    ...
    $("#email-btn").on( "click", function (e) {
        ...
        var thehtml = dt.html();
        EmailModal({template: "default", subject: "Installment Payment Listing", title:"Installment Payment Listing", html: thehtml});
    });
}]);