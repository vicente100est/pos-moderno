window.angularApp.controller("PmethodController", [
    "$scope",
    "API_URL",
    "window",
    "jQuery",
...
function (
    ...
    dt.dataTable({
        ...
        "columnDefs": [
            ...
            { 
                ...
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#pmethod-pmethod-list thead tr th:eq(0)").html());
                }
            },
            { 
                "targets": [1],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#pmethod-pmethod-list thead tr th:eq(1)").html());
                }
            },
            { 
                "targets": [2],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#pmethod-pmethod-list thead tr th:eq(2)").html());
                }
            },
            { 
                "targets": [3],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#pmethod-pmethod-list thead tr th:eq(3)").html());
                }
            },
            { 
                "targets": [4],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#pmethod-pmethod-list thead tr th:eq(4)").html());
                }
            },
            { 
                "targets": [5],
        ...
        "buttons": [
            ...
            },
            {
                ...
                titleAttr: "Copy",
                title: window.store.name + " > Payment Methods",
                exportOptions: {
                    columns: [ 0, 1, 2, 3, 4 ]
                }
            },
    ...
    $(document).delegate("#create-pmethod-submit", "click", function(e) {
        e.preventDefault();
        var $tag = $(this);
    ...
    // Edit pmethod method
    $(document).delegate("#edit-pmethod", "click", function(e) {
        e.stopPropagation();
        e.preventDefault();
    ...
    $(document).delegate("#delete-pmethod", "click", function(e) {
        e.stopPropagation();
        e.preventDefault();
        var datatable = dt;
        var d = datatable.DataTable().row( $(this).closest("tr") ).data();
        PmethodDeleteModal(d);
    });

    // Oopen edit modal dialog box by query string
    if (window.getParameterByName("pmethod_id") && window.getParameterByName("pmethod_name")) {
        pmethodId = window.getParameterByName("pmethod_id");
        var customerName = window.getParameterByName("pmethod_name");
        dt.DataTable().search(customerName).draw();
        dt.DataTable().ajax.reload(function(json) {
            $.each(json.data, function(index, obj) {
                if (obj.DT_RowId === "row_" + pmethodId) {
                    PmethodEditModal({pmethod_id: pmethodId, pmethod_name: obj.pmethod_name});
                    return false;
                }
            });
        }, false);
