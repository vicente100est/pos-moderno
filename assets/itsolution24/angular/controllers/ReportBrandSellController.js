window.angularApp.controller("ReportBrandSellController", [
    "$scope",
    "API_URL",
    "window",
    "jQuery",
...
function (
    ...
    var $brandid = window.getParameterByName("brand_id");
    ...
    dt.dataTable({
        ...
        "dom": "lfBrtip",
        "serverSide": true,
        "ajax": API_URL + "/_inc/report_sell_the_brandwise.php?from="+$from+"&to="+$to+"&brandid="+$brandid,
        "fixedHeader": true,
        "order": [[ 0, "desc"]],
        "aLengthMenu": [
            [10, 25, 50, 100, 200, -1],
        ...
        "buttons": [
            {
                ...
                titleAttr: "Print",
                title: "Sell Report (Brand wise)-"+from+" to "+to,
            ...
            },
            {
                ...
                titleAttr: "Copy",
                title: window.store.name + " > Sell Report (Brand wise)-"+from+" to "+to,
                exportOptions: {
                    columns: [printColumns]
                }
            },
            {
                ...
                titleAttr: "Excel",
                title: window.store.name + " > Supplier Sell Controller",
                exportOptions: {
                    columns: [printColumns]
                }
            },
    ...
    $(document).delegate("#brand_id", "select2:select", function (e) {
        e.preventDefault();
        e.stopPropagation();
        var data = e.params.data;
        window.location = window.baseUrl+"/admin/brand_profile.php?brand_id="+data.element.value;
    });

    if (window.getParameterByName('brand_id')) {
        $("#brand_id").val(window.getParameterByName('brand_id')).trigger("change");
    }
