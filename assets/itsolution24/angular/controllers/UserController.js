window.angularApp.controller("UserController", [
    "$scope", 
    "API_URL",
    "window",
    "jQuery",
...
function ($scope,
    ...
    dt.dataTable({
        ...
        "columnDefs": [
            ...
            { 
                "targets": [9],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#user-user-list thead tr th:eq(9)").html());
                }
            },
        ],
        "aoColumns": [
        ...
        "buttons": [
            ...
            },
            {
                ...
                titleAttr: "Copy",
                title: window.store.name + " > User List",
                exportOptions: {
                    columns: [ 0, 1, 2, 3, 4, 5 ]
                }
            },
    ...
    $(document).delegate("#create-user-submit", "click", function(e) {
        ...
        then(function(response) {
            ...
            dt.DataTable().ajax.reload(function(json) {
                if ($("#row_"+userId).length) {
                    $("#row_"+userId).flash("yellow", 5000);
                }
            }, false);
    ...
    if (window.getParameterByName("user_id") && window.getParameterByName("username")) {
        userId = window.getParameterByName("user_id");
        var customerName = window.getParameterByName("username");
        dt.DataTable().search(customerName).draw();
        dt.DataTable().ajax.reload(function(json) {
            $.each(json.data, function(index, obj) {
                if (obj.DT_RowId === "row_" + userId) {
                    UserEditModal({id: userId, username: obj.username});
                    return false;
                }
            });
        }, false);
    ...
    $("#email-btn").on( "click", function (e) {
        e.stopPropagation();
        e.preventDefault();
        dt.find("thead th:nth-child(8), thead th:nth-child(9), thead th:nth-child(10), tbody td:nth-child(8), tbody td:nth-child(9), tbody td:nth-child(10), tfoot th:nth-child(8), tfoot th:nth-child(9), tfoot th:nth-child(10)").addClass("hide-in-mail");
        var thehtml = dt.html();
        EmailModal({template: "default", subject: "User Listing", title:"User Listing", html: thehtml});
    });
