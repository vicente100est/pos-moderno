window.angularApp.factory("CustomerAddBalanceModal", ["API_URL", "window", "jQuery", "$http", "$uibModal", "$sce", "$rootScope", function (API_URL, window, $, $http, $uibModal, $sce, $scope) {
    return function(customer) {
        var uibModalInstance = $uibModal.open({
            ...
            template: "<div class=\"modal-header\">" +
                            "<button id=\"add-balance-modal-close\" ng-click=\"closeCustomerAddBalanceModal();\" type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-label=\"Close\"><span aria-hidden=\"true\">&times;</span></button>" +
            ...
            controller: function ($scope, $uibModalInstance) {
                $http({
                  url: window.baseUrl + "/_inc/customer.php?customer_id=" + customer.customer_id + "&action_type=ADDBALANCE",
                  method: "GET"
                })
                ...
                $(document).delegate("#submit-balance", "click", function(e) {
                    ...
                    then(function(response) {
                        ...
                        .then(function (willDelete) {
                            $("#add-balance-modal-close").trigger("click");
                        });
